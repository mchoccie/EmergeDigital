...
class MatchMaking:
    ...
    def update_the_dataset(self, client, new_profile, df, id):
        ...
        #to_add = dictionary[0]
        #to_add['Age'] = str(to_add['Age'])
        change_set = {'Primary goal': str(new_profile.iloc[0]['Primary goal']), 'Secondary goal': str(new_profile.iloc[0]['Secondary goal']), 'Industry': str(new_profile.iloc[0]['Industry']), 'Gender': str(new_profile.iloc[0]['Gender']), 'Age': str(new_profile.iloc[0]['Age']), 'Role': str(new_profile.iloc[0]['Role'])}
        #_id = client.test.ai.insert_one(to_add)
        result = client.test.ai.update_one({'uid': str(id)}, {'$set': change_set})
...
def connect():
    ...
    client = MongoClient(
        environ.get('MONGODB_URL'), ssl_cert_reqs=ssl.CERT_NONE)
    return client
    ...
    if new_user is not None:
        ...
        if the_role == 1 and the_type == 1:
            ...
            if matched_coach is not None:
                ...
                if is_existing == 0:
                    ...
                    if client.test.matched.find_one({'Leader id': str(sys.argv[7])}) is None:
                        client.test.matched.insert_one(to_insert)
                        print('Saved this user in matched table')
                    #things = client.test.ai.find_one({'_id': ObjectId('6185f944088fcfa5800e13c2')})
                    ...
                    #_id = client.test.ai.insert_one(to_add)
                        result = client.test.matched.update_one({'uid': str(sys.argv[7])}, {'$set': change_set})
        ...
        # if he/she is a coach, just save this new profile to the dataset
            ...
            if is_existing == 0:
                ...
                new_user.update_the_dataset(
                    client, new_profile, df_ai, sys.argv[7])
                print('Updated a coach')
